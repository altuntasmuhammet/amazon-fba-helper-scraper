...
def get_product_prices_from_product_page(response):
    ...
    for element_id in main_price_element_ids:
        element_text = ' '.join([s.strip() for s in sel.xpath(
            f"//*[@id='{element_id}']//text()").extract() if s.strip()])
        if element_text:
            element_text = element_text.replace("¥", "")
            element_text = element_text.replace("$", "")
            element_text = element_text.replace("TL", "")
            # logging.info(f"get_product_prices_from_product_page.main_price_element_text: {element_text}")
            main_price = extract_number_with_two_decimals(element_text)
            # logging.info(f"get_product_prices_from_product_page.main_price: {main_price}")
    ...
    for element_id in shipping_price_element_ids:
        ...
        if element_text:
            element_text = element_text.replace(",", "")
            # logging.info(f"get_product_prices_from_product_page.shipping_price_element_text: {element_text}")
    ...
    # logging.info(f"get_product_prices_from_product_page.mode_main_price: {main_price}")
    # logging.info(f"get_product_prices_from_product_page.mode_shipping_price: {shipping_price}")
    if main_price is not None and shipping_price is not None:
        ...
        for element_id in total_price_element_ids:
            ...
            if '+' in element_text or not 'hipping' in element_text:
...
def get_lowest_product_prices_from_tr_sellers_page(response):
    ...
    for offer_element in offer_elements:
        track_message_element = offer_element.xpath(
            ".//*[@id='fast-track-message']")
        if not track_message_element:
            track_message_element = offer_element.xpath(
                ".//*[@class='a-row aod-delivery-promise']")
        if track_message_element:
            cannot_be_shipped_element = track_message_element.xpath(
                ".//*[contains(., 'cannot be shipped') or @class='a-color-error']")
            if cannot_be_shipped_element:
                continue
            else:
                # Whole
                whole_price_text = offer_element.xpath(
                    ".//*[@class='a-price-whole']/text()").get()
                ...
                # Fraction
                fraction_price_text = offer_element.xpath(
                    ".//*[@class='a-price-fraction']/text()").get().strip()
                ...
                shipping_price_element_texts_2 = offer_element.xpath(
                    ".//*[contains(@class, 'aod-ship-charge')]//text()").extract()
                shipping_price_element_text_2 = ' '.join(
                    [s.strip() for s in shipping_price_element_texts_2 if s.strip()])
                shipping_price_element_texts_3 = offer_element.xpath(
                    ".//*[@class='a-row aod-delivery-promise']//text()").extract()
                shipping_price_element_text_3 = ' '.join(
                    [s.strip() for s in shipping_price_element_texts_3 if s.strip()])
                shipping_price_element_texts_4 = offer_element.xpath(
                    ".//*[contains(@id,'unified-delivery-message-')]//text()").extract()
                shipping_price_element_text_4 = ' '.join(
                    [s.strip() for s in shipping_price_element_texts_4 if s.strip()])
                shipping_price_element_texts_5 = offer_element.xpath(
                    ".//*[@class='a-row aod-ship-charge']//text()").extract()
                shipping_price_element_text_5 = ' '.join(
                    [s.strip() for s in shipping_price_element_texts_5 if s.strip()])
                shipping_price_element_text = shipping_price_element_text_1 + shipping_price_element_text_2 + \
                    shipping_price_element_text_3 + \
                    shipping_price_element_text_4 + shipping_price_element_text_5
...
def has_no_buybox_in_us_product_page(response):
    sel = Selector(response)
    see_buying_options_button = sel.xpath(
